...
export async function ensureSchema() {
  ...
  await pool.query(`
    ...
    -- ── Theory Vault ─────────────────────────────────────────────────────────
    -- Completely separate from mednexus_questions. Each row is one TheoryQuestion.
    -- The data JSONB column holds the full TheoryQuestion payload (prompt,
    -- modelAnswer, criticalFlags, pastPapers, tags) as a single document so the
    -- schema can evolve without ALTER TABLE.  Scalar columns (category, module,
    -- set_number) are promoted to real columns for efficient server-side filtering.
    CREATE TABLE IF NOT EXISTS mednexus_theory_questions (
      id          TEXT        PRIMARY KEY,          -- UUID generated by the client/admin
      category    TEXT        NOT NULL DEFAULT '',  -- e.g. "Pharmacology"
      module      TEXT        NOT NULL DEFAULT '',  -- e.g. "Level 400 Clinicals"
      set_number  INTEGER     NOT NULL DEFAULT 1,   -- ordered group within the module
      data        JSONB       NOT NULL DEFAULT '{}',-- full TheoryQuestion payload
      created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
    CREATE INDEX IF NOT EXISTS idx_theory_questions_module
      ON mednexus_theory_questions (module);
    CREATE INDEX IF NOT EXISTS idx_theory_questions_category
      ON mednexus_theory_questions (category);
