...
export function sanitizeTheoryMedia(value: unknown): TheoryMediaItem[] {
  ...
  const sanitized = value.flatMap((entry, index) => {
    if (!entry || typeof entry !== "object") throw new Error(`Image ${index + 1} is invalid.`)
    const record = entry as Record<string, unknown>
    const type: TheoryMediaItem["type"] = record.type === "diagram" ? "diagram" : "image"
  ...
  if (sanitized.length > THEORY_MEDIA_MAX_ITEMS) throw new Error(`A question can contain at most ${THEORY_MEDIA_MAX_ITEMS} images.`)
  return sanitized
}
