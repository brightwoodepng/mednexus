...
import {
  AlertTriangle, CheckCircle2, ChevronDown, Copy, Download, Filter, Grid2X2,
  ImageIcon, List, Loader2, Move, Plus, RefreshCw, Search, SquarePen, Tags,
  Trash2, Upload, X,
} from "lucide-react"
import type { Question } from "@/lib/types"

type ManagedQuestion = Question & { status: "draft" | "review" | "live" | "offline" | "archived"; validationIssues: string[]; mediaCount: number }
type ListResponse = { questions: ManagedQuestion[]; pagination: { page: number; pageSize: number; total: number; pages: number }; filters: { modules: string[]; subjects: string[] }; counts: Record<string, number>; updatedAt: string | null }
...
const emptyData: ListResponse = { questions: [], pagination: { page: 1, pageSize: 18, total: 0, pages: 1 }, filters: { modules: [], subjects: [] }, counts: {}, updatedAt: null }
...
export function McqModernWorkspace({ onOpenImporter }: { onOpenImporter: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : "Unable to load the MCQ bank.")
  ...
  const exportUrl = useMemo(() => {
    const params = new URLSearchParams({ bank: "mcq", format: "json" }); selected.forEach((id) => params.append("id", id)); return "/api/admin/content/export?" + params
  }, [selected])
  ...
  function saveView() {
    ...
    if (!name) return
    setSavedViews((current) => [...current.filter((view) => view.name !== name), { name: name.slice(0, 40), filters }].slice(-8))
  }
  ...
  async function bulk(action: string, payload: Record<string, unknown> = {}) {
    ...
    if (action === "delete" && !window.confirm(`Permanently delete ${selected.size} selected MCQs? This cannot be undone.`)) return
    setSaving(true); setError("")
    try {
      ...
      setBulkOpen(false); setMoveModule(""); setMoveSubject(""); setTagText(""); await load()
  ...
  const summaryItems: Array<[string, number, () => void, boolean]> = [
    ["All", ["live", "draft", "review", "offline", "archived"].reduce((sum, key) => sum + Number(data.counts[key] ?? 0), 0), () => clearFilters(), activeFilterCount === 0],
    ...
    <section className="rounded-xl border border-border bg-card shadow-sm">
      <div className="flex flex-col gap-3 border-b border-border p-3 lg:flex-row lg:items-center">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex">
          ...
          <button onClick={() => void load()} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border px-3 text-sm font-semibold"><RefreshCw size={16}/>Refresh</button>
          <button onClick={onOpenImporter} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-border px-3 text-sm font-semibold"><Upload size={16}/>Import</button>
      ...
      <div className={`${filtersOpen ? "grid" : "hidden"} gap-2 border-b border-border p-3 sm:grid-cols-2 lg:grid xl:grid-cols-6`}>
        <select value={filters.moduleName} onChange={(e) => setFilter("moduleName", e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-sm"><option value="">All modules</option>{data.filters.modules.map((item) => <option key={item}>{item}</option>)}</select>
        <select value={filters.subject} onChange={(e) => setFilter("subject", e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-sm"><option value="">All disciplines</option>{data.filters.subjects.map((item) => <option key={item}>{item}</option>)}</select>
        <select value={filters.status} onChange={(e) => setFilter("status", e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-sm"><option value="">All statuses</option><option value="live">Live</option><option value="draft">Draft</option><option value="review">In review</option><option value="offline">Offline</option><option value="archived">Archived</option></select>
        <select value={filters.media} onChange={(e) => setFilter("media", e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-sm"><option value="">Any media</option><option value="with">With images</option><option value="without">Without images</option></select>
        <select value={filters.sort} onChange={(e) => setFilter("sort", e.target.value)} className="h-10 rounded-lg border border-border bg-background px-3 text-sm"><option value="updated-desc">Recently updated</option><option value="updated-asc">Oldest updated</option><option value="module">Module</option><option value="stem">Question stem</option></select>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-3 py-2.5 text-sm">
        ...
        <span className="text-muted-foreground">{data.pagination.total} questions · {selected.size} selected{data.updatedAt ? ` · Updated ${new Date(data.updatedAt).toLocaleString()}` : ""}</span>
        <div className="flex items-center gap-2"><div className="inline-flex rounded-lg border border-border p-0.5"><button onClick={() => setLayout("grid")} aria-label="Grid view" aria-pressed={layout === "grid"} className={`rounded-md p-2 ${layout === "grid" ? "bg-muted text-primary" : "text-muted-foreground"}`}><Grid2X2 size={15}/></button><button onClick={() => setLayout("list")} aria-label="List view" aria-pressed={layout === "list"} className={`rounded-md p-2 ${layout === "list" ? "bg-muted text-primary" : "text-muted-foreground"}`}><List size={15}/></button></div><button disabled={!selected.size} onClick={() => setBulkOpen(true)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-border px-3 font-semibold disabled:opacity-40"><Filter size={15}/>Bulk actions</button></div>
      </div>
    </section>
...
function QuestionResult({ question, selected, layout, onToggle }: { question: ManagedQuestion; selected: boolean; layout: Layout; onToggle: () => void }) {
  ...
  return <article className={`overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-[border-color,box-shadow] hover:border-primary/40 hover:shadow-md ${layout === "list" ? "flex min-h-32" : ""}`}>
    {preview ? <div className={`${layout === "list" ? "hidden w-36 sm:block" : "h-32"} shrink-0 overflow-hidden bg-muted`}><img src={preview} alt={mediaItem?.alt || "Question image"} className="h-full w-full object-cover"/></div> : layout === "grid" && <div className="flex h-16 items-center justify-center bg-primary/5"><ImageIcon className="text-primary/40"/></div>}
    <div className={`flex min-w-0 flex-1 flex-col gap-2 p-3 ${layout === "list" ? "sm:flex-row sm:items-center" : ""}`}><input type="checkbox" checked={selected} onChange={onToggle} aria-label={`Select ${question.vignette}`} className="shrink-0"/><div className="min-w-0 flex-1"><div className="flex flex-wrap gap-1.5"><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold capitalize ${statusStyles[question.status] || statusStyles.draft}`}>{question.status}</span>{question.mediaCount > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-violet-500/10 px-2 py-0.5 text-[10px] text-violet-700 dark:text-violet-300"><ImageIcon size={10}/>{question.mediaCount}</span>}</div><p className={`${layout === "grid" ? "mt-2 line-clamp-3 min-h-[3.75rem]" : "mt-1 line-clamp-2"} text-sm font-semibold leading-5`}>{question.vignette || "Untitled draft question"}</p><p className="mt-1 truncate text-xs text-muted-foreground">{question.module || "Unassigned module"} · {question.subject || "Unassigned discipline"}</p>{question.validationIssues.length ? <p className="mt-2 flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-300"><AlertTriangle size={13}/>{question.validationIssues.length} issue{question.validationIssues.length === 1 ? "" : "s"}: {question.validationIssues[0]}</p> : <p className="mt-2 flex items-center gap-1.5 text-xs text-emerald-700 dark:text-emerald-300"><CheckCircle2 size={13}/>Ready to publish</p>}</div><Link href={`/admin/mcq/${question.id}`} className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-lg bg-primary/10 px-3 text-sm font-semibold text-primary hover:bg-primary hover:text-primary-foreground"><SquarePen size={15}/>Open</Link></div>
  </article>
}
