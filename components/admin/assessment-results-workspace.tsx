...
type Payload = { summaries: Summary[]; total: number; modules: string[]; metrics: { assessments: number; participants: number; average: number; passRate: number } }
...
export function AssessmentResultsWorkspace() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const params = new URLSearchParams({ search, module: moduleName, status })
    const response = await fetch(`/api/admin/results?${params}`)
    const body = await response.json()
    if (response.ok) setData(body)
    setLoading(false)
  }, [search, moduleName, status])
