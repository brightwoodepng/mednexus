...
Continue the same numbered structure for every remaining question under the single Pathology discipline. PREAMBLE, MODEL ANSWER, KEY POINTS, and IMAGES are optional: include them only when they already exist in the source document. Keep every A/B/C item on its own line and use matching A/B/C headings in MODEL ANSWER.`
  ...
  return <div className="space-y-5">
    ...
    {stage === "preview" && <section className={card}>
      <div className="flex flex-wrap items-start justify-between gap-3"><div><span className="rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-bold text-amber-700">Not saved yet</span><h2 className="mt-3 text-xl font-bold">Import preview</h2><p className="mt-1 text-sm text-muted-foreground">{items.length} valid questions across {groups.length} hierarchy paths. Review them, then use the save button below to add them to the Theory Vault.</p></div><button onClick={reset} className={`${button} border border-border`}>Start over</button></div>
      ...
      <div className="mt-5 max-h-[42rem] space-y-3 overflow-y-auto pr-1">{items.map((item, index) => <article key={`${item.prompt}-${index}`} className="rounded-xl border border-border p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-xs font-bold uppercase tracking-wide text-primary">{[item.collectionTitle,item.moduleName,item.disciplineName].filter(Boolean).join(" / ")}</p><h3 className="mt-1 font-bold">{item.title}</h3><TheoryMarkdown children={item.prompt} className="mt-2 text-muted-foreground"/>{item.modelAnswer?<details className="mt-3 rounded-lg border border-border bg-muted/20 p-3"><summary className="cursor-pointer text-xs font-bold">Preview structured model answer</summary><TheoryMarkdown children={item.modelAnswer} className="mt-2"/></details>:null}{item.modelAnswer && item.keyMarkingPoints.length ? <p className="mt-2 text-xs font-semibold text-primary">{item.marks} marks · {item.keyMarkingPoints.length} key points · Automatically allocated draft</p> : <p className="mt-2 text-xs font-semibold text-amber-600 dark:text-amber-400">Prompt only · Publish for practice while the model answer is prepared</p>}</div><button onClick={() => setItems(current => current.filter((_, itemIndex) => itemIndex !== index))} aria-label="Remove from import" className="rounded-lg p-2 text-destructive hover:bg-destructive/10"><Trash2 size={16}/></button></div>{item.media.length > 0 && <div className="mt-3"><TheoryQuestionMedia media={item.media} compact/></div>}</article>)}</div>
      <div className="mt-5 flex justify-end"><button disabled={!items.length} onClick={commit} className={`${button} bg-primary text-primary-foreground disabled:opacity-50`}><CheckCircle2 size={16}/>Save {items.length} drafts to Theory Vault</button></div>
    </section>}
  </div>
}
