    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background overflow-hidden">
      ...
      <div className="shrink-0 flex items-center gap-2 border-b border-border bg-card px-3 py-2.5">
        ...
        <div className="flex items-center gap-1">
          ...
          <button type="button" onClick={() => setShowNav((v) => !v)}
            className={`flex md:hidden h-8 w-8 items-center justify-center rounded-lg border transition-colors ${showNav ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted text-muted-foreground hover:bg-muted/80"}`}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-4 pt-4 pb-24 sm:px-6 sm:pt-6 sm:pb-24 md:pb-6 lg:px-8 lg:pt-8 lg:pb-8">
            ...
            <div className="hidden md:flex items-center justify-between pt-1">
              <button type="button" onClick={() => setCurrentIdx((i) => Math.max(0, i - 1))} disabled={currentIdx === 0}
                className="flex min-h-[44px] items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                <ChevronLeftIcon size={15} /> Previous
              </button>
              {currentIdx < questions.length - 1 ? (
                <button type="button" onClick={() => setCurrentIdx((i) => Math.min(questions.length - 1, i + 1))}
                  className="flex min-h-[44px] items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm">
                  Next <ChevronRightIcon size={15} />
                </button>
              ) : (
                <button type="button" disabled={submitting || submitted} onClick={requestSubmit}
                  className="flex min-h-[44px] items-center gap-2 rounded-xl bg-emerald-600 px-5 py-2 text-sm font-semibold text-white hover:bg-emerald-700 transition-colors shadow-sm disabled:opacity-60">
        ...
        {/* ── Desktop navigator sidebar (md+) ── */}
        <div className="hidden md:flex w-56 shrink-0 flex-col border-l border-border bg-muted/30 p-3 overflow-y-auto">
          <NavPanel
      ...
      {/* ── Mobile sticky bottom nav (< md) ── */}
      <div className="md:hidden shrink-0 flex items-center justify-between gap-3 border-t border-border bg-card px-4 py-3">
        <button type="button" onClick={() => setCurrentIdx((i) => Math.max(0, i - 1))} disabled={currentIdx === 0}
          className="flex min-h-[44px] items-center gap-2 rounded-xl border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        {currentIdx < questions.length - 1 ? (
          <button type="button" onClick={() => setCurrentIdx((i) => Math.min(questions.length - 1, i + 1))}
            className="flex min-h-[44px] items-center gap-2 rounded-xl bg-primary px-5 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors shadow-sm">
            Next <ChevronRightIcon size={15} />
          </button>
        ) : (
          <button type="button" disabled={submitting || submitted} onClick={requestSubmit}
