...
export function TutorialOverlay({ tutorial, stepIndex, isPhone, measureEpoch, interactionComplete, interactionAnchorId, onStep, onComplete, onPause, onDismiss }: { tutorial: TutorialDefinition; stepIndex: number; isPhone: boolean; measureEpoch: number; interactionComplete: boolean; interactionAnchorId?: string; onStep: (step: number) => void; onComplete: () => void; onPause: () => void; onDismiss: () => void }) {
  ...
  const content = <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby="tutorial-title" aria-describedby="tutorial-body" data-testid="tutorial-dialog" className="min-w-0" aria-live="polite">
    <TutorialProgress current={stepIndex} total={tutorial.steps.length}/><p className="mt-1.5 text-[10px] font-bold uppercase tracking-widest text-primary sm:mt-2 sm:text-[11px]">{tutorial.name}</p><h2 id="tutorial-title" className="mt-0.5 text-base font-bold leading-5 sm:text-lg sm:leading-6">{confirmSkip ? "Stop this guided tour?" : step.title}</h2>{!confirmSkip && <p id="tutorial-body" className="mt-1 text-[13px] leading-[1.125rem] text-muted-foreground sm:mt-1.5 sm:text-sm sm:leading-5">{step.body}</p>}
    {confirmSkip ? <div id="tutorial-body" className="mt-2"><p className="text-[13px] leading-[1.125rem] text-muted-foreground sm:text-sm sm:leading-5">Resume later from this step, or turn this tutorial off. Your study progress is never affected.</p><div className="mt-2 grid grid-cols-2 gap-2"><button className="min-h-11 rounded-lg border bg-card px-2 text-sm font-semibold" onClick={onPause}>Resume later</button><button className="min-h-11 rounded-lg bg-destructive px-2 text-sm font-semibold text-destructive-foreground" onClick={onDismiss}>Turn off</button><button className="col-span-2 min-h-10 rounded-lg text-sm font-semibold text-muted-foreground" onClick={() => setConfirmSkip(false)}>Keep going</button></div></div> : <>
    ...
    <div className="mt-2.5 grid grid-cols-2 gap-1.5 sm:mt-3 sm:flex sm:flex-wrap"><button className="min-h-11 rounded-xl border px-3 text-sm font-semibold" disabled={stepIndex === 0} onClick={() => onStep(stepIndex - 1)}>Previous</button>{stepIndex === tutorial.steps.length - 1 ? <button className="min-h-11 rounded-xl bg-primary px-3 text-sm font-bold text-primary-foreground sm:flex-1" onClick={onComplete}><TutorialCompletion label={tutorial.finishLabel}/></button> : <button disabled={!interactionComplete} className="min-h-11 rounded-xl bg-primary px-3 text-sm font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50 sm:flex-1" onClick={() => onStep(stepIndex + 1)}>Next</button>}<button className="col-span-2 min-h-10 rounded-xl px-3 text-sm font-semibold text-muted-foreground sm:col-span-1 sm:min-h-11" onClick={() => setConfirmSkip(true)}>Skip tutorial</button></div>
    <button className="min-h-10 w-full text-sm text-muted-foreground sm:min-h-11" onClick={onPause}>Close and resume later</button></>}
  </div>
  ...
  return <div className="pointer-events-none fixed inset-0 z-[80] h-[100dvh]" data-testid="tutorial-overlay"><div className={`absolute inset-0 bg-foreground/55 ${allowTargetInteraction ? "pointer-events-none" : "pointer-events-auto"}`} aria-hidden/><button aria-label="Close and resume tutorial later" className={`absolute inset-0 cursor-default ${allowTargetInteraction ? "pointer-events-none" : "pointer-events-auto"}`} onClick={onPause}/><TutorialSpotlight rect={rect}/>{isPhone ? <TutorialMobileSheet avoidBottomNavigation={Boolean(bottomTarget)}>{content}</TutorialMobileSheet> : <TutorialCoachmark style={coachStyle}>{content}</TutorialCoachmark>}</div>
}
