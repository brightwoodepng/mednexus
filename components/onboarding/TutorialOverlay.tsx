...
export function TutorialOverlay({ tutorial, stepIndex, isPhone, measureEpoch, interactionComplete, onStep, onComplete, onPause, onDismiss }: { tutorial: TutorialDefinition; stepIndex: number; isPhone: boolean; measureEpoch: number; interactionComplete: boolean; onStep: (step: number) => void; onComplete: () => void; onPause: () => void; onDismiss: () => void }) {
  ...
  const content = <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby="tutorial-title" aria-describedby="tutorial-body" data-testid="tutorial-dialog" className="min-w-0" aria-live="polite">
    ...
    {confirmSkip ? <div className="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-2.5"><p className="text-sm font-semibold">Stop this guided tour?</p><div className="mt-2 grid grid-cols-2 gap-2"><button className="min-h-11 rounded-lg border bg-card px-2 text-sm" onClick={() => setConfirmSkip(false)}>Keep going</button><button className="min-h-11 rounded-lg bg-destructive px-2 text-sm font-semibold text-destructive-foreground" onClick={onDismiss}>Don&apos;t show again</button><button className="col-span-2 min-h-11 rounded-lg px-2 text-sm font-semibold text-muted-foreground" onClick={onPause}>Skip for now</button></div></div> : <div className="mt-3 grid grid-cols-2 gap-2"><button className="min-h-11 rounded-xl border px-3 text-sm font-semibold" disabled={stepIndex === 0} onClick={() => onStep(stepIndex - 1)}>Previous</button>{stepIndex === tutorial.steps.length - 1 ? <button className="min-h-11 rounded-xl bg-primary px-3 text-sm font-bold text-primary-foreground" onClick={onComplete}><TutorialCompletion label={tutorial.finishLabel}/></button> : <button disabled={!interactionComplete} className="min-h-11 rounded-xl bg-primary px-3 text-sm font-bold text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50" onClick={() => onStep(stepIndex + 1)}>Next</button>}<button className="min-h-11 rounded-xl px-2 text-sm font-semibold text-muted-foreground" onClick={onPause}>Resume later</button><button className="min-h-11 rounded-xl px-2 text-sm font-semibold text-muted-foreground" onClick={() => setConfirmSkip(true)}>Skip tutorial</button></div>}
  </div>
