...
import {
  ...
  FolderOpen, MoveRight, Pencil, Plus, RefreshCw, Search, Settings2,
  Trash2, X,
} from "lucide-react"
...
type Tab = "editor" | "import" | "imports" | "trash"
...
export function TheoryAdminSimplified({ initialTab = "editor" }: { initialTab?: Tab }) {
  ...
  const effectiveSetSize = setSize ?? data?.settings.defaultSetSize ?? 20
  ...
  return <div className="space-y-4 pb-12">
    ...
    <nav className="flex gap-2 overflow-x-auto rounded-xl border border-border bg-card p-2">{([
      ["editor","Questions",BookOpen],["import","Import file",FileUp],["imports","Import history",FolderOpen],["trash","Trash",Trash2],
    ...
    {tab === "editor" ? <div className="grid gap-4 xl:grid-cols-[300px_1fr]">
      <aside className={`${card} h-fit overflow-hidden`}>
        <div className="border-b border-border p-4"><div className="flex items-start justify-between gap-2"><div><h2 className="font-bold">{kind === "end_of_module" ? "Modules" : "Disciplines"}</h2><p className="text-xs text-muted-foreground">Choose a folder to see its sets.</p></div><button onClick={async()=>{const name=window.prompt(`New ${kind === "end_of_module" ? "module" : "discipline"} name`);const collectionId=data.collections[0]?.id;if(name&&collectionId)await change("POST",{resource:kind === "end_of_module" ? "module" : "discipline",collectionId,name},"Folder created.")}} aria-label="Add folder" className="rounded-lg border border-border p-2 text-primary"><Plus size={15}/></button></div><label className="mt-3 flex items-center gap-2 text-xs font-semibold text-muted-foreground">Questions per new set<select value={effectiveSetSize} onChange={e=>setSetSize(Number(e.target.value))} className="ml-auto rounded-lg border border-border bg-background px-2 py-1.5 text-foreground">{[15,16,17,18,19,20].map(value=><option key={value}>{value}</option>)}</select><button disabled={busy||effectiveSetSize===data.settings.defaultSetSize} onClick={()=>void change("PATCH",{action:"settings",defaultSetSize:effectiveSetSize},"Set size saved.")} className="rounded-lg bg-primary px-2 py-1.5 text-primary-foreground disabled:opacity-40">Save</button></label></div>
    ...
    {settingsGroup ? <FolderSettingsPanel group={settingsGroup} sets={data.sets.filter(item=>kind==="end_of_module"?item.moduleId===settingsGroup.id:item.disciplineId===settingsGroup.id)} kind={kind} busy={busy} onClose={()=>setSettingsGroup(null)} change={change} trash={trash}/> : null}
  </div>
}
...
function FolderSettingsPanel({group,sets,kind,busy,onClose,change,trash}:{group:Group;sets:SetRow[];kind:Kind;busy:boolean;onClose:()=>void;change:(method:"POST"|"PATCH",body:Record<string,unknown>,success:string)=>Promise<boolean>;trash:(resource:string,ids:string[])=>Promise<void>}) {
  const [target,setTarget]=useState("")
  const [name,setName]=useState(group.name)
  const [status,setStatus]=useState<Status>("draft")
  const selectedSet=sets.find(item=>item.id===target)
  const targetLabel=selectedSet?.name??group.name
  const resource=selectedSet?"set":kind==="end_of_module"?"module":"discipline"
  const scope=selectedSet?{setId:selectedSet.id}:kind==="end_of_module"?{moduleId:group.id}:{disciplineId:group.id}
  const chooseTarget=(id:string)=>{setTarget(id);setName(sets.find(item=>item.id===id)?.name??group.name)}
  const rename=async()=>{if(!name.trim())return;if(await change("PATCH",{resource,id:selectedSet?.id??group.id,name:name.trim()},`${selectedSet?"Set":"Folder"} renamed.`))onClose()}
  const applyStatus=async()=>{
    if(!window.confirm(`Change every question in ${targetLabel} to ${status === "published" ? "Live" : status}?`))return
    if(selectedSet&&!await change("PATCH",{resource:"set",id:selectedSet.id,status},"Set status updated."))return
    if(await change("PATCH",{action:"bulk",operation:status,scope},`All questions in ${targetLabel} updated.`))onClose()
  }
  const remove=async()=>{onClose();await trash(resource,[selectedSet?.id??group.id])}
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-3" onClick={onClose}><section role="dialog" aria-modal="true" aria-label={`Settings for ${group.name}`} onClick={event=>event.stopPropagation()} className={`${card} w-full max-w-lg p-5`}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-primary">Folder settings</p><h2 className="mt-1 text-xl font-bold">{group.name}</h2><p className="mt-1 text-sm text-muted-foreground">Manage the whole {kind==="end_of_module"?"module":"discipline"} or one of its sets.</p></div><button onClick={onClose} aria-label="Close settings" className="rounded-lg p-2 hover:bg-muted"><X/></button></div><div className="mt-5 space-y-4"><label className="block text-xs font-bold text-muted-foreground">Apply changes to<select value={target} onChange={event=>chooseTarget(event.target.value)} className={`${control} mt-1 w-full text-foreground`}><option value="">Entire {kind==="end_of_module"?"module":"discipline"}</option>{sets.map(set=><option key={set.id} value={set.id}>{set.name} ({set.questionCount} questions)</option>)}</select></label><div className="rounded-xl border border-border p-4"><label className="block text-xs font-bold text-muted-foreground">Rename {selectedSet?"set":"folder"}<input value={name} onChange={event=>setName(event.target.value)} className={`${control} mt-1 w-full text-foreground`}/></label><button disabled={busy||!name.trim()||name.trim()===targetLabel} onClick={()=>void rename()} className={`${button} mt-3 w-full border border-border disabled:opacity-40`}><Pencil size={14}/>Save name</button></div><div className="rounded-xl border border-border p-4"><label className="block text-xs font-bold text-muted-foreground">Change all question statuses<select value={status} onChange={event=>setStatus(event.target.value as Status)} className={`${control} mt-1 w-full text-foreground`}><option value="draft">Draft</option><option value="review">In review</option><option value="published">Live</option><option value="archived">Archived</option></select></label><button disabled={busy} onClick={()=>void applyStatus()} className={`${button} mt-3 w-full bg-primary text-primary-foreground disabled:opacity-40`}>Apply to all questions</button></div><button disabled={busy} onClick={()=>void remove()} className={`${button} w-full bg-destructive/10 text-destructive disabled:opacity-40`}><Trash2 size={15}/>Move {selectedSet?"set":"folder"} to Trash</button></div></section></div>
}
