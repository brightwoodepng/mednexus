...
function StudyQuestion({ questionId, sessionQuestionIds, registered, onBack, onFinish, onMove }: { questionId: string; sessionQuestionIds: string[] | null; registered: boolean; onBack: (setId: string | null) => void; onFinish: (setId: string | null) => void; onMove: (id: string) => void }) {
  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-3 pb-24 sm:space-y-4 md:pb-0">
      ...
      {/* ── Top nav bar ── */}
      <div className="sticky top-0 z-30 -mx-3 grid w-[calc(100%+1.5rem)] grid-cols-[44px_minmax(0,1fr)_40px_40px] items-center gap-1.5 border-b border-border/70 bg-background/95 px-3 pb-2 pt-2 shadow-sm backdrop-blur-md sm:static sm:mx-0 sm:flex sm:w-full sm:gap-3 sm:bg-transparent sm:px-0 sm:pb-4 sm:pt-0 sm:shadow-none sm:backdrop-blur-none">
        ...
        <div className="contents sm:flex sm:w-auto sm:items-center sm:gap-2">
          <button
            ...
            className={`flex h-10 items-center justify-center gap-2 rounded-xl px-2 text-sm font-bold transition-all sm:h-11 sm:px-3 ${
          ...
          <div role="group" aria-label="Study mode" className="col-span-4 mt-0.5 grid min-w-0 grid-cols-2 rounded-xl border-2 border-primary/25 bg-muted/60 p-1 sm:col-auto sm:mt-0"><button type="button" aria-pressed={mode === "review"} onClick={() => setMode("review")} className={`min-w-0 rounded-lg px-2 py-1.5 text-xs font-bold transition-colors sm:px-3 sm:py-2 sm:text-sm ${mode === "review" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground"}`}>Review</button><button type="button" aria-pressed={mode === "practice"} onClick={() => setMode("practice")} className={`min-w-0 rounded-lg px-2 py-1.5 text-xs font-bold transition-colors sm:px-3 sm:py-2 sm:text-sm ${mode === "practice" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground"}`}>Practice</button></div>
        </div>
      </div>
      ...
      <article className="min-w-0 overflow-hidden rounded-[1.25rem] border border-border bg-card p-4 shadow-sm sm:p-7">
          <div className={`grid min-w-0 items-start gap-4 sm:gap-6 ${question.media.length ? "lg:grid-cols-[minmax(0,1fr)_minmax(240px,34%)]" : ""}`}><div className="min-w-0"><div className="hidden text-xs font-bold text-muted-foreground sm:flex"><span className="rounded-full bg-muted px-3 py-1">{question.collectionTitle}</span></div><h1 className="mt-3 hidden break-words text-xl font-bold leading-snug text-foreground sm:block">{question.title || "Theory question"}</h1><TheoryMarkdown children={question.prompt} className="mt-0 text-foreground/85 sm:mt-4" linkedSectionKeys={answerLinksEnabled ? answerSectionKeys : []} onSectionSelect={answerLinksEnabled ? jumpToModelAnswer : undefined}/></div>{question.media.length > 0 && <div className="min-w-0 lg:pt-2"><TheoryQuestionMedia media={question.media}/></div>}</div>
      </article>
