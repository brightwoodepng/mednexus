  ...
  return (
    <div className="mx-auto min-w-0 max-w-7xl space-y-3 pb-24 sm:space-y-4 md:pb-0">
      ...
      <article className="min-w-0 overflow-hidden rounded-[1.25rem] border border-border bg-card p-4 shadow-sm sm:p-7">
          <div className={`grid min-w-0 items-start gap-4 sm:gap-6 ${question.media.length ? "lg:grid-cols-[minmax(0,1fr)_minmax(240px,34%)]" : ""}`}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2 text-xs font-bold text-muted-foreground">{question.marks != null && <span>{question.marks} marks</span>}<span className="hidden rounded-full bg-muted px-3 py-1 sm:inline-flex">{question.collectionTitle}</span></div><h1 className="mt-3 hidden break-words text-xl font-bold leading-snug text-foreground sm:block">{question.title || "Theory question"}</h1><TheoryMarkdown children={question.prompt} className="mt-1 text-foreground/85 sm:mt-4" linkedSectionKeys={answerLinksEnabled ? answerSectionKeys : []} onSectionSelect={answerLinksEnabled ? jumpToModelAnswer : undefined}/></div>{question.media.length > 0 && <div className="min-w-0 lg:pt-2"><TheoryQuestionMedia media={question.media}/></div>}</div>
      </article>
