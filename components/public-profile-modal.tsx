...
import { CosmeticFrame, CosmeticHighlight } from "@/components/cosmetics"
...
export function PublicProfileModal({ entry, npLabel = "Lifetime XP", onClose }: PublicProfileModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Card */}
      <CosmeticHighlight cosmeticId={entry.equippedHighlight} size="profile" motionState="focused" className="relative z-10 w-full max-w-sm rounded-3xl">
      <div role="dialog" aria-modal="true" aria-label={`${entry.name}'s profile`} className="max-h-[calc(100dvh-1.5rem)] w-full overflow-y-auto rounded-3xl border border-border bg-card shadow-2xl animate-in zoom-in-95 duration-200">
