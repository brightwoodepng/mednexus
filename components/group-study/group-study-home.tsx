...
export function GroupStudyHome() {
  ...
  useEffect(() => {
    if (available > 0 && questionCount > available) setQuestionCount(available)
  }, [available, questionCount])

  async function createRoom() {
    setBusy(true); setError("")
    try {
      const room = await multiplayerApi<{ pin: string }>("/api/group-study", { method: "POST", body: JSON.stringify({ moduleId, discipline, questionCount, timerSeconds }) })
  ...
  return <main className="min-h-screen bg-background text-foreground"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[1.5fr_1fr]">
      <section className="rounded-3xl border bg-card p-5 shadow-sm sm:p-7"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Plus size={19}/></span><div><h2 className="font-bold">Create a room</h2><p className="text-xs text-muted-foreground">Up to 10 registered members</p></div></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="space-y-2 text-sm font-semibold sm:col-span-2">Module<select value={moduleId} onChange={event => { const next = modules.find(module => module.id === event.target.value); setModuleId(event.target.value); setDiscipline(""); setQuestionCount(Math.min(30, next?.total ?? 30)) }} className="h-11 w-full rounded-xl border bg-background px-3 font-normal"><option value="">Choose a module</option>{modules.map(module => <option key={module.id} value={module.id}>{module.id} ({module.total})</option>)}</select></label>
          ...
          {selected && <label className="space-y-2 text-sm font-semibold sm:col-span-2">Number of questions<input type="number" min={1} max={available} value={questionCount} onChange={event => setQuestionCount(Math.max(1, Math.min(available, Number(event.target.value) || 1)))} className="h-11 w-full rounded-xl border bg-background px-3 font-normal"/><span className="block text-xs font-normal text-muted-foreground">Choose between 1 and {available}. Unseen questions are selected first.</span></label>}
