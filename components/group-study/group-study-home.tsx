...
export function GroupStudyHome() {
  ...
  return <main className="min-h-screen bg-background text-foreground"><div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10">
    ...
    <div className="mt-6 grid gap-5 md:grid-cols-2">
      <section className="flex min-h-72 flex-col rounded-3xl border bg-card p-5 shadow-sm sm:p-7"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Plus size={19}/></span><div><h2 className="font-bold">Create a room</h2><p className="text-xs text-muted-foreground">{canCreate ? "Host up to 10 registered or guest participants" : "Guests can join rooms; sign in to create one"}</p></div></div><p className="mt-6 text-sm leading-6 text-muted-foreground">Choose your module, number of questions, navigation mode and optional answer timer.</p><button type="button" disabled={!canCreate} onClick={() => setCreateDialogOpen(true)} className="mt-auto flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary font-bold text-primary-foreground shadow-sm transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"><Plus size={18}/>{canCreate ? "Create a room" : "Registered account required"}</button></section>
    ...
    {createDialogOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm" onMouseDown={event => { if (event.target === event.currentTarget && !busy) setCreateDialogOpen(false) }}><section role="dialog" aria-modal="true" aria-labelledby="create-room-title" className="max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-3xl border bg-card p-5 shadow-2xl sm:p-7"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary"><Plus size={19}/></span><div className="min-w-0 flex-1"><h2 id="create-room-title" className="font-bold">Create a room</h2><p className="text-xs text-muted-foreground">Host up to 10 registered or guest participants</p></div><button type="button" disabled={busy} onClick={() => setCreateDialogOpen(false)} aria-label="Close create room dialog" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-muted-foreground transition hover:bg-muted hover:text-foreground disabled:opacity-50"><X size={20}/></button></div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_2fr]"><button type="button" disabled={busy} onClick={() => setCreateDialogOpen(false)} className="h-12 rounded-xl border font-bold transition hover:bg-muted disabled:opacity-50">Cancel</button><button disabled={busy || !canCreate || !moduleId || available < 1 || questionCount < 1 || questionCount > available} onClick={createRoom} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-primary font-bold text-primary-foreground shadow-sm transition hover:opacity-90 disabled:opacity-50"><Users size={18}/>{busy ? "Working…" : "Create Group Study room"}</button></div></section></div>}
  </div></main>
}
