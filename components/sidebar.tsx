...
export function Sidebar({screen,onNavigate,mobileOpen,onCloseMobile,collapsed,onCollapse,onExpand}: SidebarProps) {
 const {user,cloudEnabled,signOutUser}=useApp(); const {isGlassEnabled}=useTheme(); const {activeStudyHub,setActiveStudyHub}=useApplicationShell(); const nav=(s:Screen)=>{onNavigate(s);onCloseMobile()}; const navigation=useMemo(()=>getHubNavigation(activeStudyHub),[activeStudyHub]); const canAdmin=user?.serverRole === "ADMIN" || user?.serverRole === "SUPER_ADMIN"
