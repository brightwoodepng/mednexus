...
import { ArrowDown, ArrowUp, BookOpen, FolderTree, History, Plus, Save, Search, Settings2, Trash2 } from "lucide-react"
...
type QuestionRow = {
  ...
  referencesMd: string; media: Array<Record<string, unknown>>; tags: string[]; status: Status; sortOrder: number
}
type AdminData = {
...
type Tab = "hierarchy" | "questions" | "settings" | "audit"
...
export function TheoryAdminManager({ initialTab = "questions" }: { initialTab?: Tab }) {
  ...
  if (!data) return <div className={`${card} py-16 text-center text-sm text-muted-foreground`}>{error || "Loading Theory administration…"}</div>
  const tabs: Array<[Tab, string, typeof BookOpen]> = [["hierarchy","Curriculum",FolderTree],["questions","Questions",BookOpen],["settings","Settings",Settings2],["audit","Audit",History]]
...
function QuestionForm({ question, data, onClose, change }: { question: QuestionRow | null; data: AdminData; onClose: () => void; change: (method: "POST" | "PATCH" | "DELETE", body: Record<string, unknown>, success: string) => Promise<boolean> }) {
  ...
  const [mediaText, setMediaText] = useState(JSON.stringify(question?.media ?? [], null, 2))
  ...
  const save = async () => {
    ...
    try {
      ...
      window.alert("Media metadata must be a valid JSON array.")
      return
    }
    const payload = { collectionId, moduleId:collection?.kind==="end_of_module"?groupId:null, disciplineId:collection?.kind==="end_of_year"?groupId:null, setId:setId||null, autoAssign:!setId, title,prompt,modelAnswer,keyMarkingPoints:points.split("\n").map(x=>x.trim()).filter(Boolean),tags:tags.split(",").map(x=>x.trim()).filter(Boolean),referencesMd,media,marks:marks?Number(marks):null,status }
  ...
  return <section className={card}><div className="flex justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-primary">{question ? "Edit question" : "New question"}</p><h2 className="mt-1 text-xl font-bold">{question?.title || "Create Theory question"}</h2></div><button onClick={onClose} className="text-sm font-bold text-muted-foreground">Close</button></div><div className="mt-5 grid gap-4 lg:grid-cols-3"><label className="text-xs font-bold text-muted-foreground">Category<select value={collectionId} onChange={event => setCollectionId(event.target.value)} className={`${control} mt-1 w-full text-foreground`}>{data.collections.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label className="text-xs font-bold text-muted-foreground">{collection?.kind==="end_of_module"?"Module":"Discipline"}<select value={groupId} onChange={event => setGroupId(event.target.value)} className={`${control} mt-1 w-full text-foreground`}>{groups.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-xs font-bold text-muted-foreground">Set<select value={setId} onChange={event => setSetId(event.target.value)} className={`${control} mt-1 w-full text-foreground`}><option value="">Auto-assign next available set</option>{sets.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
...
function SettingsAdmin({ value, change }: { value: number; change: (method: "POST" | "PATCH" | "DELETE", body: Record<string, unknown>, success: string) => Promise<boolean> }) {
  ...
  return <section className={`${card} max-w-xl`}><h2 className="text-xl font-bold">Set assignment</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">New questions without a selected set are assigned to the next set with capacity. When every set is full, a new set is created automatically.</p><label className="mt-5 block text-sm font-bold">Default set size<select value={size} onChange={event => setSize(Number(event.target.value))} className={`${control} mt-2 w-full`}>{[15,16,17,18,19,20].map(value => <option key={value}>{value}</option>)}</select></label><button onClick={() => change("PATCH",{action:"settings",defaultSetSize:size},"Theory settings saved.")} className={`${button} mt-4 bg-primary text-primary-foreground`}><Save size={16}/>Save settings</button></section>
}

function Audit({ items }: { items: AdminData["audit"] }) {
  return <section className={card}><h2 className="text-xl font-bold">Recent Theory activity</h2>{items.length ? <div className="mt-4 divide-y divide-border">{items.map(item => <div key={item.id} className="grid gap-1 py-3 sm:grid-cols-[160px_1fr_auto] sm:items-center"><b className="capitalize">{item.action.replaceAll("_"," ")}</b><span className="text-sm text-muted-foreground">{item.resourceType} {item.resourceId ? `· ${item.resourceId}` : ""}</span><time className="text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleString()}</time></div>)}</div> : <p className="mt-3 text-sm text-muted-foreground">No Theory administration changes have been recorded yet.</p>}</section>
}
