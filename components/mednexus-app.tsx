    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="glass-modal relative w-full max-w-sm overflow-hidden rounded-3xl border border-border bg-card shadow-2xl">
        ...
        <div className="relative flex items-center gap-4 bg-primary px-5 py-5 text-primary-foreground">
          ...
          <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-white/20 shadow-lg">
            <StethoscopeIcon size={28} />
          </div>
          <div className="relative min-w-0">
            <h2 className="text-xl font-bold tracking-tight text-white">MedNexus</h2>
            <p className="text-xs" style={{ color: "rgba(255,255,255,0.7)" }}>Medical Study Platform</p>
          </div>
        ...
        <div className="space-y-4 px-5 py-5">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-border bg-muted/40 px-4 py-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-foreground">Britechinc</p>
              <p className="text-xs text-muted-foreground">Built by a medical student, for medical students</p>
            </div>
          </div>
          ...
          <div className="space-y-2.5">
            ...
            {/* General WhatsApp Card */}
            <a
              href="https://wa.me/233543982307"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 rounded-2xl border border-[#a8d5b5] bg-[#e6f4ea] px-4 py-3 transition-opacity hover:opacity-80"
            >
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#c8e6c9]">
                <svg viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5 text-[#1b5e20]">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-[#1b5e20]">Chat on WhatsApp</p>
                <p className="text-xs text-[#1b5e20]">For support contact admin</p>
              </div>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4 shrink-0 text-[#1b5e20]">
            ...
            <a
              href="https://wa.me/233543982307?text=Hi%20Britechinc,%20I%20want%20to%20support%20MedNexus"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-start gap-3 rounded-2xl border border-[#d4aa87] bg-[#fdf0e4] px-4 py-3 transition-opacity hover:opacity-80"
            >
              <HeartIcon size={16} className="mt-0.5 shrink-0 text-[#5c3317]" />
              <div className="min-w-0">
                <p className="text-sm font-bold text-[#5c3317]">Support &amp; Donations</p>
                <p className="mt-0.5 text-xs leading-relaxed text-[#5c3317]">
...
function TheoryStudyModeToggle({ mode, onChange }: { mode: TheoryStudyMode; onChange: (mode: TheoryStudyMode) => void }) {
  return <div role="group" aria-label="Theory study mode" className="grid grid-cols-2 rounded-xl border-2 border-primary/25 bg-muted/60 p-1 md:hidden">
    ...
    <button type="button" aria-pressed={mode === "practice"} onClick={() => onChange("practice")} className={`rounded-lg px-2.5 py-1.5 text-xs font-bold transition-colors ${mode === "practice" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground"}`}>Practice</button>
  </div>
}
...
export function MedNexusApp() {
  ...
  const [theoryStudyMode, setTheoryStudyMode] = useState<TheoryStudyMode>("review")
  ...
  return (
    ...
    >
          ...
          {safeScreen === "theory-dashboard" && <TheoryVault key={`theory-dashboard-${theoryNavigationKey}`} initialView="Dashboard" initialDashboard={getRecentTheoryDashboard()} externalQuery={theorySearchQuery} onExternalQueryChange={setTheorySearchQuery} onQuestionViewChange={setTheoryQuestionOpen} studyMode={theoryStudyMode} onStudyModeChange={setTheoryStudyMode} />}
          {safeScreen === "theory-browse" && <TheoryVault key={`theory-browse-${theoryNavigationKey}`} initialView="Browse Questions" externalQuery={theorySearchQuery} onExternalQueryChange={setTheorySearchQuery} onQuestionViewChange={setTheoryQuestionOpen} studyMode={theoryStudyMode} onStudyModeChange={setTheoryStudyMode} />}
          {safeScreen === "theory-bookmarks" && <TheoryVault key={`theory-bookmarks-${theoryNavigationKey}`} initialView="Bookmarks" externalQuery={theorySearchQuery} onExternalQueryChange={setTheorySearchQuery} onQuestionViewChange={setTheoryQuestionOpen} studyMode={theoryStudyMode} onStudyModeChange={setTheoryStudyMode} />}
          {safeScreen === "theory-notes" && <TheoryVault key={`theory-notes-${theoryNavigationKey}`} initialView="My Notes" externalQuery={theorySearchQuery} onExternalQueryChange={setTheorySearchQuery} onQuestionViewChange={setTheoryQuestionOpen} studyMode={theoryStudyMode} onStudyModeChange={setTheoryStudyMode} />}
