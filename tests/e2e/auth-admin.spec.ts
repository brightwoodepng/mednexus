...
test("public visitors only see authentication actions", async ({ page }) => {
  ...
  await page.goto("/")
  await expect(page.getByRole("region", { name: "MedNexus dashboard on a laptop with Rankings on a phone" })).toBeVisible()
  await expect(page.getByTestId("auth-reference-art")).toBeVisible()
  ...
  await expect(page.getByTestId("auth-reference-art")).toBeHidden()
  await expect(page.getByRole("heading", { name: "Welcome back" })).toBeVisible()
