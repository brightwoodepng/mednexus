...
test("public visitors only see authentication actions", async ({ page }) => {
  ...
  const otpButton = page.getByRole("button", { name: "Forgot password? Enter with OTP" })
  ...
  await expect(guestButton).toHaveClass(/auth-btn-secondary/)
  await expect(guestButton).toHaveCSS("box-shadow", "none")
  await expect(otpButton).toHaveCSS("text-decoration-line", "none")
  await expect(createAccountButton).toHaveClass(/auth-link/)
  await expect(createAccountButton).toHaveCSS("text-decoration-line", "none")
  await expect(supportLink).toHaveClass(/auth-link/)
  await expect(supportLink).toHaveCSS("text-decoration-line", "none")
  await createAccountButton.hover()
  await expect(createAccountButton).toHaveCSS("text-decoration-line", "underline")
