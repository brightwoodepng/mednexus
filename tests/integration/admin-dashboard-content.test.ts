...
describe("admin dashboard and content workflows", () => {
  ...
  it("hosts both importers inside Imports and Exports", async () => {
    ...
    expect(source).toContain("Choose where these questions belong.")
    expect(source).toContain('setTheoryKind("end_of_module")')
    expect(source).toContain('setTheoryKind("end_of_year")')
