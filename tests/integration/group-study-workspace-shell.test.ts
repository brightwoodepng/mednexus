...
describe("Group Study learner workspace", () => {
  it("keeps the shared learner sidebar on desktop room routes", async () => {
    ...
    expect(groupStudyHome).toContain("Study better together")
    expect(groupStudyHome).toContain('htmlFor="group-study-pin"')
    expect(groupStudyHome).toContain('className="mt-2 flex gap-2"')
