...
describe("economy season management",()=>{
  ...
  it("manages reward rules and audited NT gifts by learner index number",()=>{
    for(const action of ['action==="update_rules"','action==="gift_nt"'])expect(api).toContain(action)
    expect(api).toContain("applyNPCredits")
    expect(api).toContain("LOWER(index_number)=LOWER($1)")
    ...
    for(const label of ["Finalize month","Save reward rules","Gift NT to a learner","Index number","Send NT gift","All-time XP remains continuous across seasons"])expect(ui).toContain(label)
    expect(ui).not.toContain("Finalize week")
  ...
  it("archives the prior season and verifies every opening balance",()=>{
    ...
    expect(ui).toContain("NT balances, purchases, cosmetics, and lifetime XP remain")
