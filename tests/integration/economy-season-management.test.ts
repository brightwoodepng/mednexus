...
const api = read("app/api/admin/economy-seasons/route.ts"),
  ui = read("components/admin/economy-seasons-workspace.tsx"),
  settingsApi = read("app/api/admin/settings/route.ts"),
  settingsUi = read("components/admin/system-settings-workspace.tsx"),
  economy = read("lib/economy-seasons.ts");
...
describe("economy season management", () => {
  ...
  it("plans and activates seasons only through protected confirmed actions", () => {
    ...
    expect(api).toContain(
      "A planned season already uses this name or economy version.",
    );
  });
  ...
  it("archives the prior season and verifies every opening balance", () => {
    for (const text of [
      "mednexus_economy_season_archives",
      "status='closed'",
      "status='active'",
      "mednexus_economy_cutovers",
      "Opening balance verification failed",
      "auditAdmin",
    ])
  ...
  it("saves system and theory settings atomically with strict validation", () => {
    ...
    expect(settingsApi).toContain(
      "SELECT id FROM mednexus_system_settings WHERE id=1 FOR UPDATE",
    );
