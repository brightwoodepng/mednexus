...
describe("Theory Vault phone experience", () => {
  ...
  it("includes phone reflow and unobscured question navigation contracts", async () => {
    ...
    expect(source).toContain("grid-cols-[44px_minmax(0,1fr)_40px_40px]")
    expect(source).toContain("contents sm:flex")
    expect(source).toContain('className="hidden sm:inline">{state?.bookmark')
    expect(source).toContain('className="hidden sm:inline">{state?.revision')
  ...
  it("uses a focused phone question header and finishes back at the current set", async () => {
    ...
    expect(question).toContain("contents sm:flex sm:w-auto")
    ...
    expect(question).toContain("{question.collectionTitle}")
    expect(question).toContain("hidden text-xs font-bold text-muted-foreground sm:flex")
    expect(question).toContain('className="sm:hidden"')
    expect(question).toContain("hidden break-words text-xl font-bold")
    ...
    expect(question).toContain("col-span-4 mt-0.5")
    expect(question).toContain('aria-pressed={mode === "review"}')
    expect(question).toContain("w-[calc(100%+1.5rem)]")
    expect(question).not.toContain('px-3 py-1">Question {question.position}</span>')
