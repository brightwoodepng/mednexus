...
describe("Theory Vault phone experience", () => {
  ...
  it("includes phone reflow and unobscured question navigation contracts", async () => {
    ...
    expect(source).toContain("space-y-3 pb-24 sm:space-y-4")
    ...
    expect(source).toContain("overflow-x-auto border-b")
    expect(source).toContain("[scrollbar-width:none] [&::-webkit-scrollbar]:hidden")
    expect(source).toContain('className="flex min-w-max items-center gap-2 md:min-w-0"')
    expect(source).toContain('className="hidden min-w-0 grid-cols-2')
  ...
  it("uses a focused phone question header and finishes back at the current set", async () => {
    ...
    expect(question).toContain("md:overflow-visible")
    ...
    expect(question).toContain("Question {question.position}")
    expect(question).toContain('className="hidden shrink-0 md:inline"')
    expect(question).toContain('aria-hidden className="hidden h-4 w-px shrink-0 bg-primary/40 md:block"')
    expect(question).toContain("inline-flex max-w-full items-center gap-2 rounded-full")
    expect(question).toContain("min-w-0 truncate font-bold text-primary")
    expect(question).toContain("min-w-0 overflow-hidden rounded-[1.25rem]")
    expect(question).toContain('role="group" aria-label="Study mode"')
    expect(question).toContain('aria-pressed={mode === "review"}')
    expect(question).toContain("w-[calc(100%+1.5rem)]")
    expect(question.match(/\{question\.marks\} marks/g)).toHaveLength(1)
    ...
    expect(app).toContain('aria-label="Theory study mode"')
    expect(app).toContain('activeStudyHub === "theory-vault" && theoryQuestionOpen')
    expect(app).toContain('<TheoryStudyModeToggle mode={theoryStudyMode} onChange={setTheoryStudyMode} />')
    expect(app).toContain('className="grid grid-cols-2 rounded-xl border-2 border-primary/25 bg-muted/60 p-1 md:hidden"')
  })
  ...
  it("keeps the dashboard, set browser, and study review compact", async () => {
    ...
    expect(source).toContain('className="hidden text-sm text-muted-foreground md:block"')
    expect(source).toContain('bg-primary/5 px-3 py-3 text-left')
    expect(source).toContain('emphasizedOnMobile')
    expect(source).toContain('aria-label="Set study summary" className="border-b border-border pb-4"')
    expect(source).toContain('grid grid-cols-3 divide-x divide-border rounded-xl bg-muted/50')
    expect(source).toContain('grid-cols-[minmax(52px,0.8fr)_auto_minmax(52px,0.8fr)]')
