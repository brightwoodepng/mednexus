...
describe("database transfer regressions", () => {
  ...
  it("externalizes newly persisted inline MCQ media when object storage is configured", () => {
    ...
    const importJobs = read("app/api/admin/content/imports/route.ts")
    const importJob = read("app/api/admin/content/imports/[id]/route.ts")
    ...
    expect(normalizer).not.toContain('import sharp from "sharp"')
    expect(normalizer.indexOf("questions.some(question => question.mediaBase64)")).toBeLessThan(normalizer.indexOf('await import("sharp")'))
    ...
    expect(importJobs).not.toContain("ensureSchema")
    expect(importJob).not.toContain("ensureSchema")
  ...
  it("uses version-aware startup sync and cached poll authorization", () => {
    ...
    expect(app).toContain("applyPendingMutations(remote.progress, pendingMutations.current)")
    expect(app).toContain("saveLocal(account.uid, reconciled)")
