...
function formatIndexNumber(raw) {
    ...
    if (match) {
        ...
        return {
            formatted: `sm/${type}/${year}/${seq}`,
            autoApprove: true
        };
    }
    return {
        formatted: raw.trim().toLowerCase(),
        autoApprove: false
    };
}
...
async function POST(req) {
    try {
        ...
        const existing = await pool.query("SELECT uid FROM mednexus_registered_users WHERE index_number = $1", [
            formatted
        ]);
        ...
        await pool.query(`INSERT INTO mednexus_registered_users (uid, name, level, index_number, password_hash, status)
       VALUES ($1, $2, $3, $4, $5, $6)`, [
            uid,
            name.trim(),
            level?.trim() ?? "",
        ...
        await pool.query(`INSERT INTO mednexus_users (uid, name) VALUES ($1, $2) ON CONFLICT (uid) DO NOTHING`, [
            uid,
            name.trim()
        ]);
        if (!autoApprove) {
            const notifId = `notif-reg-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
            await pool.query(`INSERT INTO mednexus_notifications (id, title, body, type, admin_only)
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0t1gdaj._.js.map