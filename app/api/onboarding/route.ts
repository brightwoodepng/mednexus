...
const bodySchema = z.object({
  ...
  currentStep: z.number().int().min(0).max(40).optional(),
  currentStepId: z.string().max(100).optional(),
})

const select = `SELECT tutorial_id AS "tutorialId", tutorial_version AS "tutorialVersion", status,
 current_step AS "currentStep", current_step_id AS "currentStepId", started_at AS "startedAt", completed_at AS "completedAt",
...
export async function POST(request: Request) {
  ...
  const { tutorialId, tutorialVersion, action, currentStep = 0, currentStepId = null } = parsed.data
  ...
  try {
    ...
    (user_id, tutorial_id, tutorial_version, status, current_step, current_step_id, started_at, completed_at, dismissed_at, updated_at)
    VALUES ($1,$2,$3,$4,$5,$7,NOW(),CASE WHEN $4='completed' THEN NOW() END,CASE WHEN $4='dismissed' THEN NOW() END,NOW())
    ON CONFLICT (user_id,tutorial_id,tutorial_version) DO UPDATE SET
      ...
      current_step_id = CASE WHEN $6='complete' AND mednexus_user_onboarding.status='completed' THEN mednexus_user_onboarding.current_step_id ELSE EXCLUDED.current_step_id END,
      ...
      updated_at = NOW()
    RETURNING tutorial_id AS "tutorialId", tutorial_version AS "tutorialVersion", status, current_step AS "currentStep", current_step_id AS "currentStepId",
      started_at AS "startedAt", completed_at AS "completedAt", dismissed_at AS "dismissedAt", updated_at AS "updatedAt"`,
    [auth.uid, tutorialId, tutorialVersion, status, step, action, action === "restart" ? null : currentStepId])
