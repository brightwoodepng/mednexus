...
export async function GET(req: NextRequest) {
  ...
  if (bank === "mcq") {
    ...
    records = (result.rows[0]?.data ?? []).filter((question: Record<string, unknown>) =>
      ...
      && (!statusFilter || question.moduleStatus === statusFilter))
  } else {
  ...
  await auditAdmin(pool, admin.uid, "export", `${bank}_content`, null, { format, records: records.length, moduleFilter, statusFilter })
